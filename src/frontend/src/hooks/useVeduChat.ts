...
export function useVeduChat({ onBotResponse }: UseVeduChatProps) {
  ...
  const bookingQuestions = [
    ...
    { key: 'date', question: 'When would you like to schedule your appointment? (Please provide the date in YYYY-MM-DD format)' },
    { key: 'time', question: 'What time would you prefer? (Please provide time in HH:MM format, e.g., 14:30)' },
    { key: 'service', question: `Which service are you interested in? Please choose from:\n${services.map((s, i) => `${i + 1}. ${s}`).join('\n')}` },
    { key: 'message', question: 'Is there anything specific you would like to address during the consultation? (Optional - you can skip this)' },
  ];
    ...
    switch (key) {
      case 'name':
        if (userInput.trim().length < 2) {
          validationError = 'Please provide a valid name with at least 2 characters.';
        }
        break;
      case 'email':
        if (!validateEmail(userInput)) {
          validationError = 'Please provide a valid email address (e.g., name@example.com).';
        }
        break;
      case 'phone':
      ...
      case 'date':
        if (!validateDate(userInput)) {
          validationError = 'Please provide a valid future date in YYYY-MM-DD format (e.g., 2026-03-15).';
        }
        break;
      case 'time':
        if (!validateTime(userInput)) {
          validationError = 'Please provide a valid time in HH:MM format (e.g., 14:30).';
        }
        break;
      case 'service':
        ...
        } else if (!services.some(s => s.toLowerCase().includes(userInput.toLowerCase()))) {
  ...
  const getServiceInfo = (serviceName: string): string => {
    const serviceDetails: { [key: string]: string } = {
      numerology: 'Numerology Reading analyzes your birth chart and numbers to provide insights into your life path, strengths, challenges, and opportunities. Our comprehensive consultation is priced at ₹5,799 and includes a detailed report. Would you like to book a session?',
      'name correction': 'Name Correction service helps align your name with favorable numerological vibrations. We analyze your current name and suggest corrections that can bring positive energy. Pricing ranges from ₹2,500 to ₹9,000 depending on complexity. Interested in learning more?',
      'lucky number': 'Lucky Number Selection identifies the most auspicious numbers for you based on your birth details and numerological chart. These numbers can be used for important decisions, dates, and more. Would you like to discover your lucky numbers?',
      reiki: 'Reiki & Energy Healing sessions promote deep relaxation, stress reduction, and energy balance. Our experienced practitioners help clear energy blockages and restore harmony. Sessions are priced at ₹1,999. Would you like to schedule a healing session?',
      'business name': 'Business Name Analysis evaluates your business name for numerological compatibility and success potential. We provide recommendations for optimal business names that align with prosperity. Pricing ranges from ₹2,500 to ₹9,000. Shall we analyze your business name?',
    };
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      // Contact information
      else if (lowerMessage.includes('contact') || lowerMessage.includes('phone') || lowerMessage.includes('whatsapp')) {
        onBotResponse(
          "You can reach us at:\n\n" +
